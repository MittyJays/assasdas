const utils = {
	...
	fspaces: (number) => Math.floor(number * 100) / 100,
	...
	parseBet2: (str) => Math.floor(Number(str.replace(/(к|k)/ig, "000").replace(/(м|m)/ig, "000000"))) < 0 ? 0 : Math.floor(Number(str.replace(/(к|k)/ig, "000").replace(/(м|m)/ig, "000000"))),
